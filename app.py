...
def check_location():
    ...
    # --- OPTIMIZING ---
    # Rounding to 4 decimals (~11m precision) significantly improves stability
    # by grouping nearby pixel queries together.
    try:
        lat = round(float(data.get('lat')), 4)
        lon = round(float(data.get('lon')), 4)
    except (TypeError, ValueError):
        return jsonify({'found': False, 'source': "Invalid Coordinates"})
